...
# Инициализация Flask приложения для Railway
app = Flask(__name__)
...
async def run_bot():
    ...
    bot_app.add_handler(CommandHandler("base", cmd_base))
    bot_app.add_handler(CommandHandler("rate", cmd_rate))
    bot_app.add_handler(CommandHandler("start_report", cmd_start_report))
    bot_app.add_handler(CommandHandler("end_report", cmd_end_report))
...
if __name__ == '__main__':
    ...
    bot_thread = threading.Thread(target=lambda: __import__('asyncio').run(run_bot()), daemon=True)
    bot_thread.start()
    
    # Запускаем Flask приложение на PORT
